<?php include ("code-head.php"); ?>

<body>
<?php include("header.php"); ?>
	
	<main id="content-area">
	
		<article class="single-page portafolio-detalle">	

				<header class="page-header">
					<h2 class="text-center">Nuestros productos</h2>
				</header>	
			
			<div class="container page-content">



				<h1><span>Kit de herramientas para baterías</span></h1>

				<div class="portafolio-ampliado">
					<img src="img/prod_kit_herramientas_baterias.jpg" alt="Kit de herramientas para baterías" class="imagen-destacada">
				</div>
				
				<div class="portafolio-ampliado">
				<p><strong>Especificaciones:</strong></p>
				<ul>
					<li></li>
				</ul>
				</div>

			</div><!-- /.cols -->

			<section id="productos-home" class="container brdr-top">
			<h2>Otros productos</h2>
			<div class="cols cf">
				
				<article>
					<div class="producto">
						<a href="#"><img src="img/foto_aura.jpg"></a>
						<a href="calidad.php">Ver m&aacute;s</a>
					</div><!-- /.icono -->

					<div class="producto">
						<a href="#"><img src="img/foto_aura.jpg"></a>
						<a href="calidad.php">Ver m&aacute;s</a>
					</div><!-- /.icono -->

					<div class="producto">
						<a href="#"><img src="img/foto_aura.jpg"></a>
						<a href="calidad.php">Ver m&aacute;s</a>
					</div><!-- /.icono -->

					<div class="producto">
						<a href="#"><img src="img/foto_aura.jpg"></a>
						<a href="calidad.php">Ver m&aacute;s</a>
					</div><!-- /.icono -->

					<div class="producto">
						<a href="#"><img src="img/foto_aura.jpg"></a>
						<a href="calidad.php">Ver m&aacute;s</a>
					</div><!-- /.icono -->

				</article>

			
		</section><!-- /#Productoss-home -->


			</div><!-- /.content -->
			
		</article><!-- /#single-page -->
	</main><!-- /#content-area -->
	
	<!-- Livemotive -->
	
	
<?php include("footer.php"); ?>
	
</body>
</html>